...
Baochip QR is a browser extension that shows the current tab's domain as a QR code. The QR payload currently uses the scheme `pwauth://pass/<domain>?time=<ISO8601-with-offset>` and refreshes once every 200ms to embed the current time.
